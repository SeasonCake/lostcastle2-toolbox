...
The toolbox installs every entry into an independent BepInEx plugin directory, verifies all registered payload files, prevents simultaneous installation of entries that provide the same DLL name, and removes only registered files during uninstall. The current v1.7.4 release catalog's 60 entries contain 61 functional payload files totaling 3,665,869 bytes and share the single pinned runtime below; generated `.cfg`, caches, interop, Doorstop, source-package contact details, and duplicate BepInEx core files are excluded.
