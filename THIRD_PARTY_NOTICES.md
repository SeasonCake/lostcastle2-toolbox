...
- The published v1.7.1 bundle remains frozen at 59 community MOD entries and 60 payload files. The current v1.7.4 release catalog has 60 entries and 61 payload files: it adds Lightning and Thunder Enhancement 1.0.1 by 脆毛肚; updates LC2 Enhancement Plan to 5.0.0 by 茶橘柚、空容、刺心、木亦, Monster Treasure to 11.7 by 懒虫桑, Reaper Summon to 2.6 and Thunder Hammer: Angry Thunder Spirit to 1.5.1 by 兔子王お, and Bobo Staff to 1.9.6 by 啊 这. The other six DLLs selected from Summon Master 2.6 are byte-identical to the 2.5 payloads and only migrate provenance. The older Reinforcement Transmitter: Lost Swordsman Shadow remains a separate entry because its plugin GUID is distinct. Superseded MonsterPlus and Summon Master 2.4/2.5 inputs are not duplicated. Max Players 1.3.0 by 梦羽 remains the exact 15,872-byte `LostCastle2MaxPlayers16.dll`; duplicated runtime and generated files remain excluded.
...
The toolbox installs every entry into an independent BepInEx plugin directory, verifies all registered payload files, prevents simultaneous installation of entries that provide the same DLL name, and removes only registered files during uninstall. The published v1.7.4–1.7.6 catalog remains frozen at 60 entries and 61 functional payload files totaling 3,665,869 bytes. The 1.7.7 source catalog contains 62 entries and 64 files totaling 3,850,189 bytes: it adds Inventory Source Panel 1.0.17 by 懒虫桑 and Niya Summon by 兔子王お, updates the former Lightning Enhancement to Lightning Build Enhancement 2.2.0 with its separate panel DLL by 脆毛肚, and updates the Lost Swordsman summon and local player/summon color components from Summon Master 3.0 by 兔子王お. This is source preparation, not a new release. Generated `.cfg`, caches, interop, Doorstop, source-package contact details, and duplicate BepInEx core files remain excluded.
...
- Bundled identity: `6.0.0-be.785+6abdba47eeebe08552282e7a58ef0f4a9ab60b62`. The current toolbox pairs it with LC2 Combat Bridge 1.7.4, a read-only local bridge for current-match combat values and final settlement correction. Detailed version history, validation limits and maintenance profiles are documented in the repository rather than repeated in the user package.
