...
| 综合主窗口与外部战斗 HUD | 真实四人长局确认本机不按 1P 猜测、离队不必然致错，并定位旧 8,192 快照整表清空；1.6.2/Bridge 0.4.12 改为最旧快照淘汰、可见降级、收窄 owner 链和 16 人合同，真实复测仍待完成 |
| MOD 管理 | v1.6.2 纳管 2 个原有工具与 53 个社区 MOD；共用一套 BepInEx，只打包 54 个最小功能载荷，支持本地自动识别、面板一键打开与添加 |
...
- v1.6.2 收录“灵魂石修改器 1.2”（作者：恨你不见）、“金币编辑器 1.0”（作者：刺心）与 53 个社区 MOD；实战/资源工具优先，外观类靠后。“纵冰之杖加强 1.16.0”“我不是药神 1.0.0 测试版”、海克斯强化 2.7.6 与纯视觉“暴击字体改紫色 1.0.0”已纳管。新增“多人房间扩容（默认 7 人）1.3.0”只携带 15,872 B 功能 DLL；偏房主建房，加入别人房间前需卸载，真实 7–16 人运行尚未验证。
...
- 所有社区 MOD 共用工具箱固定的 BepInEx/Unity 运行时；来源包里的重复 core、Doorstop、cache、interop 与生成配置不进入最终社区载荷。当前 53 个条目合计 54 个功能文件、3,391,437 B。
...
实时桥接见 [`game_plugins/LC2CombatBridge/README.zh-CN.md`](game_plugins/LC2CombatBridge/README.zh-CN.md)；临时研究探针见 [`game_plugins/LC2DamageProbe/README.zh-CN.md`](game_plugins/LC2DamageProbe/README.zh-CN.md)。Bridge 0.4.5–0.4.8 已闭合回蓝、容量变化、官方承伤121/实际 HP 变化119双口径和回营补满排除。真实多人又定位到 0.4.10 的 `damage_snapshot_overflow`→`missing` 链；0.4.12 改为最旧未消费快照淘汰、收窄通用 owner 关系并支持 16 人协议/UI。真实客机、远端召唤物、7–16 人房间与长局稳定性仍不外推为已验证。
