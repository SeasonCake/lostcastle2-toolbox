...
| 综合主窗口与外部战斗 HUD | Bridge 1.7.0：Mini HUD 局中显示“实时”，详细页说明为“实时估算/结算可能校正”；完整SyncEnd逐玩家值显示“官方结算”并可向上或向下校正 |
| MOD 管理 | v1.7.1 纳管 2 个原有工具与 59 个社区 MOD；共用一套 BepInEx，只打包 60 个最小功能载荷，支持本地自动识别、面板一键打开与添加 |
...
- 只有SyncEnd到达且逐玩家数据完整时才显示“官方结算”。官方值会原样覆盖估算，可能向上修正，也可能向下修正；逐击观察仍用于近10秒DPS、来源明细、承伤和资源变化。
- 正式版不默认保存逐事件日志、对局ZIP或测试归档；当前局状态只保留在内存中，并在明确的新session边界重置。
...
- v1.7.1 收录“灵魂石修改器 1.2”（作者：恨你不见）、“金币编辑器 1.0”（作者：刺心）与 59 个社区 MOD。本轮把怪物宝藏升级到 11.6、行刑者升级到召唤大师 2.5 载荷，新增“啵啵法杖 1.9.2”“磁暴线圈召唤啵啵 1.0.0”“雷神之锤召唤愤怒雷精灵 1.5.0”；同包另外五个 DLL 与旧来源逐字节相同，只迁移来源。旧“增援发信仪召唤失落剑客幽影”因插件 GUID 不同继续保留；裸 `MonsterPlus` 与召唤大师 2.4 作为被新来源覆盖的输入不重复收录。“多人房间扩容（默认 7 人）1.3.0”仍只携带 15,872 B 功能 DLL。
...
- 所有社区 MOD 共用工具箱固定的 BepInEx/Unity 运行时；来源包里的重复 core、Doorstop、cache、interop 与生成配置不进入最终社区载荷。当前 59 个条目合计 60 个功能文件、3,619,277 B。
...
实时桥接见 [`game_plugins/LC2CombatBridge/README.zh-CN.md`](game_plugins/LC2CombatBridge/README.zh-CN.md)。Bridge 1.7.0继续使用匿名身份完整性门：局中只发布可证明的逐玩家实时过程值，SyncEnd逐玩家完整值才作为官方结算原样覆盖；缺失、碰撞、额外记录或槽位不完整时整组拒绝，不用倍率、取最大值或角色/武器/道具特判补数。正式版关闭高频验收探针日志，不修改游戏状态。
