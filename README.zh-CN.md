...
1.7.7 源码已更新：MOD 来源安装、通用诊断导出及新增/升级 MOD。当前未制作实测包，也未发布；变化与验证边界见 [1.7.7 源码说明](docs/LC2_1.7.7_SOURCE_NOTES.zh-CN.md)。
...
- `Diagnostic` 是默认的本地维护构建：详细页提供暂停/继续与点击导出，匿名逐事件归档单局上限 128 MiB。`Distribution` 只在明确准备分享/发布时构建，不创建桌面对局归档，也不显示诊断控件；当前局状态只留在内存，并在明确的新 session 边界重置。两个档位均不记录玩家昵称、平台账号或聊天。
