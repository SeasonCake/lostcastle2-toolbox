...
下一阶段是计算器风格的综合主窗口：由它打开和管理按键显示、按键宏与战斗统计，不再启动后直接进入键盘悬浮窗。游戏中使用紧凑战斗 HUD，完整拆分只放在主窗口；键盘模块继续保留柔雾半镂空外观。v2 事件层把“游戏观察、数据聚合、UI 展示”分开：以后发现新的回血道具、魂石、召唤物击杀触发或法力效果，通常只需补充来源注册表和样本，不需要为每件道具写一套 UI 代码。
...
- 启动 `keyview.py`，或使用自行构建的 `失落城堡2按键显示器.exe`。
- 点击齿轮或按 `F10` 打开设置；支持最多 20 个键、配色、缩放和背景透明度。
