...
- 分离维护诊断候选与正式分享包：候选可暂停、导出匿名对局事件，正式分享包只在内存中保存当前局统计并在新局开始时清空；
...
当前稳定版：[v1.7.8](https://github.com/SeasonCake/lostcastle2-toolbox/releases/tag/v1.7.8)。完整变化、验证结果与已知边界见 [v1.7.8 发布说明](docs/LC2_1.7.8_RELEASE_NOTES.zh-CN.md)。

1.7.8已发布GitHub public-core包，包含r5恢复修订，并修正公开包首次初始化的配置生成。本轮单人实测已通过事件连续性、记录的UI心跳、归档与官方结算传递检查，不推广到所有联机或MOD组合。1.7.7各候选继续仅作诊断内测历史，之前的群分享包保持原字节。范围与使用说明见 [1.7.8发布说明](docs/LC2_1.7.8_RELEASE_NOTES.zh-CN.md)，原故障与恢复证据见 [恢复说明](docs/LC2_1.7.7_RECOVERY.zh-CN.md)。
...
| 综合主窗口与外部战斗 HUD | 工具箱 1.7.6 / Bridge 1.7.4：Mini HUD 局中显示“实时”且不展示诊断跳过；详细页说明“实时估算/结算可能校正”，并把跳过提示限定在当前区与下一区；DPS 优先使用完整实时累计差分，逐击仅作回退；进程首次续接可用完整非零 live 种子恢复造成/Boss且不制造 DPS 尖峰，局中承伤保持已验证逐击口径；单人结算 UI 的显式record通过身份门后发布造成/Boss/承伤三项官方值并结束session，record不可用时也以可信UI边界结束估算session |
| MOD 管理 | v1.7.6 目录纳管 2 个原有工具与 60 个社区 MOD；群内全量包只带 61 个最小功能载荷，GitHub public-core 只带目录元数据并要求用户自行提供原包；升级时只清理哈希完全匹配的已登记旧文件，修改过或未知文件会保留 |
...
- Mini HUD 局中个人、队友、全队总伤害与 Boss 伤害显示为“实时”；详细主界面标为“实时估算”并提示“结算可能校正”。它使用经过多人实测核对的游戏过程数据，但不承诺与稍后的结算画面完全相同。
- 只有多人 network SyncEnd 或单人结算 UI 到达，且逐玩家最终记录通过完整身份门时，才显示“官方结算”。官方值会原样覆盖估算，可能向上修正，也可能向下修正。进程首次进入正在进行的对局时允许完整非零 live 值恢复已有总量，但首个样本只建 DPS 基线；近10秒DPS只从后续正差分开始，逐击观察补最近锚点后的尾段或在 live 不可用时回退。
- “有事件跳过”是维护诊断信息，只显示在详细主界面的当前区和紧接的下一区；若没有新跳过，再进入下一区时自动清除。Mini HUD 始终保留简短状态，不显示这条诊断提示。
- `Diagnostic` 是默认的本地维护构建：详细页提供暂停/继续与点击导出，匿名逐事件归档单局上限 128 MiB。`Distribution` 不创建桌面对局归档，也不显示详细对局导出与逐事件记录控件；当前局状态只留在内存，并在明确的新 session 边界重置。1.7.7 两个档位均提供右上角“导出诊断”，用于收集支持信息与当前快照，不补造未记录的逐事件历史。战斗桥接不采集玩家昵称、平台账号或聊天。

## 按键宏

- 从右键菜单或显示设置打开“宏设置”。
- 支持按一次、按住循环、开关循环；示例方案默认全部停用，新建宏从空动作列表开始，新建/默认宏最长运行 60 秒。
- 主触发键和动作按键既可保留下拉选择，也可点击“录入”后直接按键；最长运行秒数在默认界面直接可改。
- 开关循环会在再次按下主键或到达最长运行时间时停止；循环间隔放在“更多”高级设置中。
- 宏只在 `LostCastle2.exe` 为前台时发送输入；切出游戏、游戏退出、修改配置或退出工具会停止并释放按键。
- `Ctrl + Shift + F12` 紧急停止全部宏。

## MOD 管理

- v1.7.6 当前清单收录“灵魂石修改器 1.2”（作者：恨你不见）、“金币编辑器 1.0”（作者：刺心）与 60 个社区 MOD。新增“雷击与轰雷强化 1.0.1”（作者：脆毛肚），并更新“LC2 增强计划 5.0.0”（作者：茶橘柚、空容、刺心、木亦）、“怪物宝藏 11.7”（作者：懒虫桑）、“召唤行刑者 2.6”“雷神之锤召唤愤怒雷精灵 1.5.1”（作者：兔子王お）和“啵啵法杖 1.9.6”（作者：啊 这）。召唤大师 2.6 来源包中另外六个 DLL 与 2.5 逐字节相同，只迁移来源。旧“增援发信仪召唤失落剑客幽影”因插件 GUID 不同继续保留；裸 `MonsterPlus` 与召唤大师 2.4/2.5 被新来源覆盖的输入不重复收录。“多人房间扩容（默认 7 人）1.3.0”仍只携带 15,872 B 功能 DLL。
...
- 普通插件安装到各自独占目录；安装升级和卸载只移除哈希完全匹配的当前或已登记旧文件，修改过、未知或链接目标会保留；同名 DLL 冲突会阻止重复安装。BepInEx 前置整包和游戏文件覆盖不会按普通 MOD 自动添加。
- 所有社区 MOD 共用工具箱固定的 BepInEx/Unity 运行时；来源包里的重复 core、Doorstop、cache、interop、说明中的联系方式与生成配置不进入最终社区载荷。当前 60 个条目合计 61 个功能文件、3,665,869 B。
...
构建 Windows 目录包（默认维护诊断候选；只有明确发布时才选择分享档）：
...
实时桥接见 [`game_plugins/LC2CombatBridge/README.zh-CN.md`](game_plugins/LC2CombatBridge/README.zh-CN.md)。Bridge 1.7.4 继续使用匿名身份完整性门：局中发布已证明的逐玩家造成/Boss实时过程值，承伤继续使用玩家根逐击观察；多人 network SyncEnd 或单人结算 UI 的完整最终记录通过身份门后，造成/Boss/承伤才作为官方结算原样覆盖。进程首次续接允许完整非零Damage/Boss live种子恢复已有累计，后续同进程新局仍要求归零；缺失、碰撞、额外记录或槽位不完整时不伪造官方值。可信单人结算UI即使暂时没有完整record也会以最后实时估算结束session，避免遗留假活动局。诊断档打开既有高频观测，分享档只保留低频启动、连接与错误支持信号；二者的数值路径相同且都不修改游戏状态。
