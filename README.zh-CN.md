...
当前稳定版：[v1.7.4](https://github.com/SeasonCake/lostcastle2-toolbox/releases/tag/v1.7.4)。完整变化、验证结果与已知边界见 [v1.7.4 发布说明](docs/LC2_1.7.4_RELEASE_NOTES_2026-09-04.zh-CN.md)。
...
| 综合主窗口与外部战斗 HUD | 工具箱 / Bridge 1.7.4：Mini HUD 局中显示“实时”且不展示诊断跳过；详细页说明“实时估算/结算可能校正”，并把跳过提示限定在当前区与下一区；DPS 优先使用完整实时累计差分，逐击仅作回退；进程首次续接可用完整非零 live 种子恢复造成/Boss且不制造 DPS 尖峰，局中承伤保持已验证逐击口径；单人结算 UI 的显式record通过身份门后发布造成/Boss/承伤三项官方值并结束session，record不可用时也以可信UI边界结束估算session |
| MOD 管理 | v1.7.4 目录纳管 2 个原有工具与 60 个社区 MOD；群内全量包只带 61 个最小功能载荷，GitHub public-core 只带目录元数据并要求用户自行提供原包；支持本地自动识别、面板一键打开与添加 |
...
- 从右键菜单或显示设置打开“宏设置”。
- 支持按一次、按住循环、开关循环；示例方案默认全部停用，新建/默认宏最长运行 60 秒。
