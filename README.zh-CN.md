...
1.7.7 各候选仅作诊断内测，不公开发布。r1 实测遇到队列溢出后永久停收；r3已通过原始输入回放、异常隔离、重连和静态可见检查，但后续复核发现一次尚未解释的85.863秒界面心跳空档。r4增加停顿时的主线程位置记录，用于定位，尚不能宣布该停顿已修复。当前等待真实游戏反馈，后续正式版本从 1.7.8 开始。已验证范围与原始故障见 [1.7.7 候选说明](docs/LC2_1.7.7_RELEASE_NOTES.zh-CN.md)，恢复行为见 [恢复说明](docs/LC2_1.7.7_RECOVERY.zh-CN.md)。
