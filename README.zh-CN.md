...
| 综合主窗口与外部战斗 HUD | Bridge 1.7：Mini HUD 局中显示“实时”，详细页说明为“实时估算/结算可能校正”；完整SyncEnd逐玩家值显示“官方结算”并可向上或向下校正 |
| MOD 管理 | v1.7 纳管 2 个原有工具与 56 个社区 MOD；共用一套 BepInEx，只打包 57 个最小功能载荷，支持本地自动识别、面板一键打开与添加 |
...
- v1.7 收录“灵魂石修改器 1.2”（作者：恨你不见）、“金币编辑器 1.0”（作者：刺心）与 56 个社区 MOD；本轮新增“怪物宝藏 11”“噩梦降临强化 1.1.0”“增援发信仪召唤失落剑客幽影 1.0.0”，并用召唤大师 2.1 来源更新“召唤行刑者”。同包另外五个已收录 DLL 与旧来源逐字节相同，只迁移来源，不误记为功能升级。“多人房间扩容（默认 7 人）1.3.0”仍只携带 15,872 B 功能 DLL；偏房主建房，加入别人房间前需卸载，真实 7–16 人运行尚未验证。
...
- 所有社区 MOD 共用工具箱固定的 BepInEx/Unity 运行时；来源包里的重复 core、Doorstop、cache、interop 与生成配置不进入最终社区载荷。当前 56 个条目合计 57 个功能文件、3,522,509 B。
...
实时桥接见 [`game_plugins/LC2CombatBridge/README.zh-CN.md`](game_plugins/LC2CombatBridge/README.zh-CN.md)。Bridge 1.7继续使用匿名身份完整性门：局中只发布可证明的逐玩家实时过程值，SyncEnd逐玩家完整值才作为官方结算原样覆盖；缺失、碰撞、额外记录或槽位不完整时整组拒绝，不用倍率、取最大值或角色/武器/道具特判补数。正式版关闭高频验收探针日志，不修改游戏状态。
