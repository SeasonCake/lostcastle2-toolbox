...
1.7.7 各候选仅作诊断内测，不公开发布。r1 实测遇到队列溢出后永久停收；r3已通过原始输入回放、异常隔离、重连和静态可见检查，但后续复核发现一次尚未解释的85.863秒界面心跳空档。r4在新一次停顿时采到主线程位于Tk事件循环；r5增加Windows原生消息定时器并保留有界调用栈诊断。原长停顿触发条件和真实游戏表现仍待验证，后续正式版本从 1.7.8 开始。已验证范围与原始故障见 [1.7.7 候选说明](docs/LC2_1.7.7_RELEASE_NOTES.zh-CN.md)，恢复行为见 [恢复说明](docs/LC2_1.7.7_RECOVERY.zh-CN.md)。
