...
A local 1.7.8 community sharing package has been built and checked from the r5 recovery revision. One actual solo session passed event-continuity, recorded UI-heartbeat, archive, and settlement-propagation checks; multiplayer and all MOD combinations are not covered by that result. All 1.7.7 candidates remain internal diagnostic history. See the [1.7.8 package notes](docs/LC2_1.7.8_RELEASE_NOTES.zh-CN.md) and [recovery history](docs/LC2_1.7.7_RECOVERY.zh-CN.md) for the scope and remaining limits. This local package work does not publish a new GitHub Release.
