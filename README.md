...
All 1.7.7 candidates are internal diagnostic builds. The tested r1 stopped consuming events after a queue overflow. The r3 candidate passed original-input replay, malformed-record isolation, reconnect, and static visible checks, but a later review found an unexplained 85.863-second UI heartbeat gap. The r4 instrumentation captured a subsequent delayed heartbeat inside Tk's event loop. The r5 revision adds a native Windows message timer and retains the bounded diagnostic stack capture. The original long-stall trigger and real-game behavior still need verification. The next public version will be 1.7.8 after verification. See the [candidate notes](docs/LC2_1.7.7_RELEASE_NOTES.zh-CN.md) and [recovery notes](docs/LC2_1.7.7_RECOVERY.zh-CN.md) for evidence and limitations.
